...
import kotlinx.android.synthetic.main.otp_layout.timer
...
//            VerifyOTP().execute()
        ...
        mVerfyOTP.setOnClickListener{
            mOTP = mOtpET.text.toString()
            if(!mOTP.isNullOrEmpty() && mOTP.length == 6) {
                VerifyOTP().execute()
            } else {
                ViewUtils().displayToast("Please provide a valid OTP", "failure", this, "")
            }
        }
...
//                mOtpLayout.visibility = View.GONE
    ...
    inner class RequestOTP : AsyncTask<String, String, String>() {
        ...
        override fun doInBackground(vararg params: String?): String {
            try {
                ...
                lHttpHelper = SignUpUtils().sendOtp(this@OTPVerification, lHttpHelper)
            ...
            try {
                if (!result.isNullOrEmpty()) {
                    ...
                    if (lResponseObject.containsKey("Status") && lResponseObject["Status"]!! == "Success") {
                        ...
                        ViewUtils().displayToast("Invalid Mobile Number", "failure", this@OTPVerification, "")
                    }
                } else {
                    ViewUtils().displayToast("Something went wrong", "failure", this@OTPVerification, "")
                }
            } catch (e: Exception) {
                e.printStackTrace()
            }
        ...
        override fun onPostExecute(result: String?) {
            ...
            try {
                if (!result.isNullOrEmpty()) {
                    var lMapper = ObjectMapper()
                    var lResponseObject = lMapper.readValue(result, HashMap::class.java)
                    if (lResponseObject.containsKey("Status")) {
                        if(lResponseObject.containsKey("Details") && lResponseObject["Details"]!! == "OTP Matched") {
                            if(intent.getStringExtra("action").equals("signup")) {
                                ...
                                var lIntent = Intent(this@OTPVerification, ResetPassword::class.java)
                                lIntent.putExtra("mobile", mMobileNumber)
                            ...
                            ViewUtils().displayToast("Incorrect OTP, Please try again", "failure", this@OTPVerification, "")
                        }
                    } else {
                        ViewUtils().displayToast("Something went wrong", "failure", this@OTPVerification, "")
                    }
